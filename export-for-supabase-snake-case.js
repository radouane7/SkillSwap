...
import { toSnakeCase } from './server/supabase-adapter.js';
...
function generateInsertStatement(table, row) {
  // Convertir les noms de colonnes de camelCase à snake_case
  const snakeCaseRow = {};
  for (const key in row) {
    snakeCaseRow[camelToSnake(key)] = row[key];
  }
  
  const columns = Object.keys(snakeCaseRow).join(', ');
  const values = Object.values(snakeCaseRow).map(escapeSqlString).join(', ');
...
async function exportData() {
  ...
  const filePath = path.join(exportDir, `supabase-export-snake-case_${formatDate()}.sql`);
  
  // Ouvrir le fichier en écriture
  const fileStream = fs.createWriteStream(filePath);
  
  // Écrire l'en-tête du fichier SQL
  fileStream.write('-- Export SQL pour Supabase avec conventions snake_case\n');
  ...
  for (const table of tables) {
    ...
    const tableCamelCase = table
      .replace(/_([a-z])/g, (_, letter) => letter.toUpperCase());
    
    const query = `SELECT * FROM ${tableCamelCase}`;
